...
@section('content')
    <div class="container">
        <div class="card shadow-sm">
            <div class="card-body">
                <h4 class="mb-4">Edit Surat Keterangan Harga Kepemilikan Tanah</h4>

                <form action="{{ route('surat.kepemilikantanah.update', $surat->_id) }}" method="POST">
                    @csrf
                    @method('PUT')
                        ...
                        <select name="pekerjaan" id="pekerjaan" class="form-control" required>
                            ...
                            @php
                                $jobs = [
                                    ...
                                    'IBU RUMAH TANGGA','WIRASWASTA','TENTARA NASIONAL INDONESIA (TNI)',
                                    'KEPOLISIAN RI (POLRI)','DOSEN','GURU','KEPALA DESA','PERANGKAT DESA',
                                    'Pegawai Kantor Desa','BIDAN','DOKTER','PERAWAT','PETANI/PEKEBUN PEMILIK LAHAN',
                                    'BURUH TANI/PERKEBUNAN','PEDAGANG','PEGAWAI NEGERI SIPIL (PNS)','BURUH HARIAN LEPAS',
                                    'SOPIR','KARYAWAN BUMN','PENSIUNAN','PEMBANTU RUMAH TANGGA','BURUH PETERNAKAN',
                                    'KONSTRUKSI','PELAUT','NELAYAN/PERIKANAN','KARYAWAN HONORER','PETERNAK','MEKANIK',
                                    'PENATA RIAS','TUKANG LAS/PANDAI BESI','INDUSTRI','USTADZ/MUBALIGH','TABIB',
                                    'BURUH NELAYAN/PERIKANAN','JURU MASAK','SENIMAN','AKUNTAN','TKI','Lainnya'
                                ];
                            @endphp
                            @foreach ($jobs as $job)
                    ...
                    <div class="mb-3">
                        ...
                        <select name="status_surat" id="status_surat" class="form-control" required>
                            ...
                            @foreach (['Pending', 'Di cek', 'Di terima', 'Ditolak'] as $status)
                                <option value="{{ $status }}" {{ old('status_surat', $surat->status_surat) == $status ? 'selected' : '' }}>
                                    {{ $status }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="status_verif" id="status_verif" class="form-control" required>
                            ...
                            @foreach (['Belum Verifikasi', 'Terverifikasi'] as $verif)
                                <option value="{{ $verif }}" {{ old('status_verif', $surat->status_verif) == $verif ? 'selected' : '' }}>
                                    {{ $verif }}
                                </option>
                            @endforeach
                        </select>
