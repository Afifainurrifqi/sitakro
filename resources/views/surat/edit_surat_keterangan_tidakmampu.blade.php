                ...
                <form action="{{ route('surat.tidakmampu.update', $suratketerangantidakmampu->_id) }}" method="POST">
                        ...
                        <select name="peruntukan_sktm" id="peruntukan_sktm" class="form-control" required>
                            ...
                            @foreach (['Biaya Pendidikan', 'Bantuan Sosial', 'Biaya Kesehatan'] as $peruntukan)
                                <option value="{{ $peruntukan }}"
                                    {{ old('peruntukan_sktm', $suratketerangantidakmampu->peruntukan_sktm) == $peruntukan ? 'selected' : '' }}>
                                    {{ $peruntukan }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="agama" id="agama" class="form-control" required>
                            ...
                            @foreach (['Islam', 'Kristen', 'Katolik', 'Hindu', 'Buddha', 'Khonghucu'] as $agama)
                                <option value="{{ $agama }}"
                                    {{ old('agama', $suratketerangantidakmampu->agama) == $agama ? 'selected' : '' }}>
                                    {{ $agama }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="status_perkawinan" id="status_perkawinan" class="form-control" required>
                            ...
                            @foreach (['Kawin', 'Belum Kawin', 'Cerai Hidup', 'Cerai'] as $status)
                                <option value="{{ $status }}"
                                    {{ old('status_perkawinan', $suratketerangantidakmampu->status_perkawinan) == $status ? 'selected' : '' }}>
                                    {{ $status }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="pekerjaan" id="pekerjaan" class="form-control" required>
                            ...
                            @foreach ($jobs as $job)
                                <option value="{{ $job }}"
                                    {{ old('pekerjaan', $suratketerangantidakmampu->pekerjaan) == $job ? 'selected' : '' }}>
                                    {{ $job }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="mb-3">
                        <label for="keterangan_fungsi_surat" class="form-label">Keterangan Fungsi Surat (Kelengkapan) <span
                                class="text-danger">*</span></label>
                    ...
                    <div class="mb-3">
                        @php
                            ...
                            $oldBantuan = old('bantuan_sosial', $suratketerangantidakmampu->bantuan_sosial ?? []);
                        @endphp
                        ...
                        <div class="mb-3">
                            ...
                            <div class="form-check">
                                @foreach ($bantuanOptions as $bantuan)
                                    <div>
                                        <input class="form-check-input" type="checkbox" name="bantuan_sosial[]"
                                            id="bantuan_{{ $loop->index }}" value="{{ $bantuan }}"
                                            {{ in_array($bantuan, (array) $oldBantuan) ? 'checked' : '' }}>
                            ...
                            <select name="status_surat" id="status_surat" class="form-control" required>
                                @foreach (['Pending', 'Di cek', 'Di terima', 'Ditolak'] as $statusSurat)
                                    <option value="{{ $statusSurat }}"
                                        {{ old('status_surat', $suratketerangantidakmampu->status_surat) == $statusSurat ? 'selected' : '' }}>
                                        {{ $statusSurat }}</option>
                                @endforeach
                            </select>
                        </div>
                            ...
                            <select name="status_verif" id="status_verif" class="form-control" required>
                                @foreach (['Belum Verifikasi', 'Terverifikasi'] as $statusVerif)
                                    <option value="{{ $statusVerif }}"
                                        {{ old('status_verif', $suratketerangantidakmampu->status_verif) == $statusVerif ? 'selected' : '' }}>
                                        {{ $statusVerif }}</option>
                                @endforeach
                            </select>
                        </div>
